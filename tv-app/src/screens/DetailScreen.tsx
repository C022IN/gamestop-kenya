...
function getMeta(item: AnyItem): string[] {
  ...
  if ('maturity_rating' in item && item.maturity_rating)
    parts.push(item.maturity_rating as string);
  return parts;
}
...
export default function DetailScreen({ route, navigation }: Props) {
  ...
  return (
    <View style={styles.container}>
      {/* Backdrop */}
      <FastImage
        source={{ uri: getBackdrop(item) || getPoster(item), priority: FastImage.priority.high }}
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.main}>
          {/* Poster */}
          <FastImage
            source={{ uri: getPoster(item), priority: FastImage.priority.normal }}
            style={styles.poster}
            resizeMode={FastImage.resizeMode.cover}
          />
