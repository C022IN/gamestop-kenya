...
import { formatDuration } from '@/utils/mediaItem';
...
function ResumeTile({
  ...
  // Show actual time watched rather than an estimated progress percentage.
  // Duration is not cached locally so we can't compute a reliable fraction.
  const watched = formatDuration(entry.positionMs);
  const subtitle =
    entry.mediaType === 'tv' && entry.season != null
      ? `S${entry.season}  E${entry.episode}${watched ? `  ·  ${watched} watched` : ''}`
      : watched ? `${watched} watched` : 'Movie';
